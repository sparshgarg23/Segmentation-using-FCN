...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    def conv_1x1(x,num_output):
        kernel_size=1
        stride=1
        return tf.layers.conv2d(x,num_output,kernel_size,stride,kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    ...
    skip_conv_1=tf.add(deconv_1,skip_layer_1)
    #Upsample
    deconv_2=tf.layers.conv2d_transpose(skip_conv_1,num_classes,4,2,'SAME',kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    #Add skip
    skip_layer_2=conv_1x1(vgg_layer3_out,num_classes)
    skip_conv_2=tf.add(deconv_2,skip_layer_2)
    deconv_3=tf.layers.conv2d_transpose(skip_conv_2,num_classes,16,8,'SAME',kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    train_op=optimize.minimize(cross_entropy_loss)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for image,label in get_batches_fn(batch_size):
            _,loss=sess.run([train_op,cross_entropy_loss],feed_dict={input_image:image,correct_label:label,keep_prob:keep_prob_stat,learning_rate:learning_rate_stat})
